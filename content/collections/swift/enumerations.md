...
title: 枚举
date: 2026-09-08
excerpt: 定义一个包含可能值的列表的自定义类型。
chapter: 语言指南
chapterOrder: 8
---

> 本文迁移自 SwiftGG 中文版《The Swift Programming Language 6.2.3》的[“枚举”](https://doc.swiftgg.team/documentation/the-swift-programming-language/enumerations/)，原文由 Apple Inc.、Swift 项目作者及 SwiftGG 译者贡献，依据 Apache License 2.0（含 Runtime Library Exception）发布。本站仅做 Markdown、链接与版式适配。
...
想了解更多相关信息，请参阅[属性](/collections/swift/properties)，[方法](/collections/swift/methods)，[构造过程](/collections/swift/initialization)，[扩展](/collections/swift/extensions)，和[协议](/collections/swift/protocols)。
...
正如在[控制流](/collections/swift/control-flow)中介绍的那样，在判断一个枚举类型的值时，`switch` 语句必须穷举所有情况。如果遗漏了 `.west` 这种情况，上面那段代码将无法通过编译，因为它没有考虑到 `CompassPoint` 的全部成员。强制穷举确保了枚举成员不会被意外遗漏。
...
在前面的例子中，使用的语法表明这个枚举遵循 [`CaseIterable`](https://developer.apple.com/documentation/swift/caseiterable) 协议。想了解协议的相关信息，请参阅[协议](/collections/swift/protocols)。
...
![](/swift-guide-assets/barcode_UPC@2x.png)

其他商品上标有 QR 码格式的二维码，它可以使用任何 ISO 8859-1 字符，并且可以编码一个最多拥有 2,953 个字符的字符串：

![](/swift-guide-assets/barcode_QR@2x.png)
...
你可以使用一个 switch 语句来检查不同的条形码类型，和[使用 Switch 语句匹配枚举值](/collections/swift/enumerations#使用-Switch-语句匹配枚举值)中的例子一样。然而，这一次，关联值可以被提取出来作为 switch 语句的一部分。你可以在 `switch` 的 `case` 分支代码中提取每个关联值作为一个常量（用 `let` 前缀）或者作为一个变量（用 `var` 前缀）来使用：
...
就像前面的 switch 语句一样，这里将 `productBarcode` 变量与模式 `.qrCode(let productCode)` 进行匹配。与 switch case 中一样，编写 `let` 会将关联值提取为常量。有关 `if`-`case` 语句的更多信息，请参阅 [模式](/collections/swift/control-flow#模式)。

## 原始值

在[关联值](/collections/swift/enumerations#关联值)小节的条形码例子中，演示了如何声明存储不同类型关联值的枚举成员。作为关联值的替代选择，枚举成员可以被默认值（称为*原始值*）预填充，这些原始值的类型必须相同。
...
上面的例子中，枚举类型 `ASCIIControlCharacter` 的原始值类型被定义为 `Character`，并设置了一些比较常见的 ASCII 控制字符。`Character` 的描述详见[字符串和字符](/collections/swift/strings-and-characters)部分。
...
>注意:
>原始值构造器是一个可失败构造器，因为并不是每一个原始值都有与之对应的枚举成员。更多信息请参阅[可失败的构造器](/collections/swift/declarations#可失败的构造器)。
...
[← 闭包](/collections/swift/closures) · [结构体和类 →](/collections/swift/classes-and-structures)
