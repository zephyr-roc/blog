...
title: 泛型形参和实参
date: 2026-09-08
excerpt: 将参数声明抽象化以脱离具体类型。
chapter: 语言参考
chapterOrder: 38
---

> 本文迁移自 SwiftGG 中文版《The Swift Programming Language 6.2.3》的[“泛型形参和实参”](https://doc.swiftgg.team/documentation/the-swift-programming-language/genericparametersandarguments/)，原文由 Apple Inc.、Swift 项目作者及 SwiftGG 译者贡献，依据 Apache License 2.0（含 Runtime Library Exception）发布。本站仅做 Markdown、链接与版式适配。

将参数声明抽象化以脱离具体类型。

本节描述泛型类型、泛型函数和泛型构造器的形参和实参。在声明泛型类型、函数或构造器时，需要指定泛型可处理的类型形参。这些类型形参相当于占位符，当实例化泛型类型或调用泛型函数、初始化器时，会被具体的类型实参替换。

关于 Swift 语言中的“泛型”，参阅 [泛型](/collections/swift/generics).
...
更多关于泛型 `where` 从句的内容和关于泛型函数声明的例子，参阅 [泛型 Where 语句](/collections/swift/generics#泛型-Where-语句).
...
正如在[泛型形参子句](/collections/swift/generic-parameters-and-arguments#泛型形参子句)中提到的，在指定泛型函数或构造器的类型实参时，不能使用泛型实参子句。
