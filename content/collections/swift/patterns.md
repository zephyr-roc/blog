...
title: 模式
date: 2026-09-08
excerpt: 模式代表单个值或者复合值的结构。例如，元组 (1, 2) 的结构是由逗号分隔的，包含两个元素的列表。因为模式代表一种值的结构，而不是特定的某个值，你可以利用模式来匹配各种各样的值。比如，(x, y) 可以匹配元组 (1, 2)，以及任何含两个元素的元组。除了利用模式匹配一个值以外，你可以从复合值中提取出部分或全部值，然后分别把各个部分的值和一个常量或变量绑定
chapter: 语言参考
chapterOrder: 37
---

> 本文迁移自 SwiftGG 中文版《The Swift Programming Language 6.2.3》的[“模式”](https://doc.swiftgg.team/documentation/the-swift-programming-language/patterns/)，原文由 Apple Inc.、Swift 项目作者及 SwiftGG 译者贡献，依据 Apache License 2.0（含 Runtime Library Exception）发布。本站仅做 Markdown、链接与版式适配。
...
如果你准备匹配的枚举用例有任何关联的值，则相应的枚举用例模式必须指定一个包含每个关联值元素的元组模式。关于使用 `switch` 语句来匹配包含关联值的枚举用例的例子，请参阅[关联值](/collections/swift/enumerations#关联值).
...
关于使用 `switch` 语句配合 `is` 模式和 `as` 模式来匹配值的例子，请参阅[Any 和 AnyObject 的类型转换](/collections/swift/type-casting#Any-和-AnyObject-的类型转换).
...
[← 特性](/collections/swift/attributes) · [泛型形参和实参 →](/collections/swift/generic-parameters-and-arguments)
