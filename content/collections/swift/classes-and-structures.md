...
title: 结构体和类
date: 2026-09-08
excerpt: 封装数据的自定义模型类型。
chapter: 语言指南
chapterOrder: 9
---

> 本文迁移自 SwiftGG 中文版《The Swift Programming Language 6.2.3》的[“结构体和类”](https://doc.swiftgg.team/documentation/the-swift-programming-language/classesandstructures/)，原文由 Apple Inc.、Swift 项目作者及 SwiftGG 译者贡献，依据 Apache License 2.0（含 Runtime Library Exception）发布。本站仅做 Markdown、链接与版式适配。
...
更多信息，请参阅 [属性](/collections/swift/properties)、[方法](/collections/swift/methods)、[下标](/collections/swift/subscripts)、[构造过程](/collections/swift/initialization)、[扩展](/collections/swift/extensions) 和 [协议](/collections/swift/protocols)。
...
更多信息，请参阅 [继承](/collections/swift/inheritance)、[类型转换](/collections/swift/type-casting)、[析构过程](/collections/swift/deinitialization) 和 [自动引用计数](/collections/swift/automatic-reference-counting)。
...
> 有关 actor 的信息，请参阅 [并发](/collections/swift/concurrency)。
...
类和结构体的初始化在 [构造过程](/collections/swift/initialization) 中有更详细的描述。
...
构造器在 [构造过程](/collections/swift/initialization) 中有更详细的描述。
...
![](/swift-guide-assets/sharedStateStruct@2x.png)
...
![](/swift-guide-assets/sharedStateClass@2x.png)
...
当你定义自己的自定义结构体和类时，
你有责任决定什么才算作两个实例相等。
定义自己的 `==` 和 `!=` 运算符实现的过程在 [等价运算符](/collections/swift/advanced-operators#等价运算符) 中有描述。
...
[← 枚举](/collections/swift/enumerations) · [属性 →](/collections/swift/properties)
