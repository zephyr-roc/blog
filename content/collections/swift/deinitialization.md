...
title: 析构过程
date: 2026-09-08
excerpt: 释放需要自定义清理的资源。
chapter: 语言指南
chapterOrder: 15
---

> 本文迁移自 SwiftGG 中文版《The Swift Programming Language 6.2.3》的[“析构过程”](https://doc.swiftgg.team/documentation/the-swift-programming-language/deinitialization/)，原文由 Apple Inc.、Swift 项目作者及 SwiftGG 译者贡献，依据 Apache License 2.0（含 Runtime Library Exception）发布。本站仅做 Markdown、链接与版式适配。
...
Swift 会在实例不再需要时自动释放它们，以释放资源。Swift 通过**自动引用计数**（*ARC*）来管理实例的内存，如 [自动引用计数](/collections/swift/automatic-reference-counting) 中所述。通常，在实例被释放时不需要执行手动清理。然而，当你处理自己的资源时，可能需要进行一些额外的清理。例如，如果创建一个自定义类来打开文件并向其中写入数据，可能需要在类实例被释放之前关闭文件。
...
[← 构造过程](/collections/swift/initialization) · [可选链式调用（Optional Chaining） →](/collections/swift/optional-chaining)
