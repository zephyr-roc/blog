...
> 本文迁移自 SwiftGG 中文版《The Swift Programming Language 6.2.3》的[“错误处理”](https://doc.swiftgg.team/documentation/the-swift-programming-language/errorhandling/)，原文由 Apple Inc.、Swift 项目作者及 SwiftGG 译者贡献，依据 Apache License 2.0（含 Runtime Library Exception）发布。本站仅做 Markdown、链接与版式适配。
...
您可以在 `catch` 后编写一个模式，以指示该子句可以处理哪些错误。如果 `catch` 子句没有模式，该子句将匹配任何错误，并将错误绑定到名为 `error` 的局部常量。有关模式匹配的更多信息，请参阅 [模式](/collections/swift/patterns)。
...
另一种捕获多个相关错误的方式是将它们放在 `catch` 后，并用逗号分隔。例如：
...
例如，下面的代码使用了 `loadImage(atPath:)` 函数，该函数从给定的路径加载图片资源，如果无法加载图像则抛出一个错误。在这种情况下，因为图片是和应用绑定的，运行时不会有错误抛出，所以适合禁用错误传递。
...
- 在只传递由通用参数描述的错误的代码中，例如函数接收一个闭包参数并传递来自该闭包的任何错误。有关传播特定错误类型与使用 rethrows 的比较，请参阅 [再抛出函数和方法](/collections/swift/declarations#再抛出函数和方法)。
...
即使不涉及错误处理代码，您也可以使用 `defer` 语句。有关详细信息，请参阅 [延迟执行的操作Deferred Actions](/collections/swift/control-flow#延迟执行的操作-deferred-actions)。

---

[← 可选链式调用（Optional Chaining）](/collections/swift/optional-chaining) · [并发 →](/collections/swift/concurrency)
