...
> 本文迁移自 SwiftGG 中文版《The Swift Programming Language 6.2.3》的[“并发”](https://doc.swiftgg.team/documentation/the-swift-programming-language/concurrency/)，原文由 Apple Inc.、Swift 项目作者及 SwiftGG 译者贡献，依据 Apache License 2.0（含 Runtime Library Exception）发布。本站仅做 Markdown、链接与版式适配。
...
- 非结构化的子任务中的代码，之后会在 [非结构化并发](/collections/swift/concurrency#非结构化并发) 中说明
...
就和类一样，actor 也是引用类型，所以在  [类是引用类型](/collections/swift/classes-and-structures#类是引用类型) 一文中有关引用类型和值类型的对比，同时适用于类和 actor。与类不同的是，actor 在同一时刻只允许一项任务访问其可变状态，这样多个任务同时与 actor 交互时才不会产生安全性问题。举个例子，下面是一个用于记录温度的 actor：
...
你可以使用 `@globalActor` 属性定义自己的单例全局 actor，如 [globalActor](/collections/swift/attributes#globalActor) 中所述。
...
你还可以使用不可用的遵循来抑制对协议的隐式遵循，如 [Implicit Conformance to a Protocol](/collections/swift/protocols#对协议的隐式遵循) 中所述。

---

[← 错误处理](/collections/swift/error-handling) · [宏 →](/collections/swift/macros)
