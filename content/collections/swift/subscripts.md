...
> 本文迁移自 SwiftGG 中文版《The Swift Programming Language 6.2.3》的[“下标”](https://doc.swiftgg.team/documentation/the-swift-programming-language/subscripts/)，原文由 Apple Inc.、Swift 项目作者及 SwiftGG 译者贡献，依据 Apache License 2.0（含 Runtime Library Exception）发布。本站仅做 Markdown、链接与版式适配。
...
更多关于 `Dictionary` 下标的信息请参考 [访问和修改字典](/collections/swift/collection-types#访问和修改字典).
...
与函数一样，下标可以接受不同数量的参数，并且为这些参数提供默认值，如 [可变参数](/collections/swift/functions#可变参数) 和 [默认参数值](/collections/swift/functions#默认参数值)中所述。但是，与函数不同的是，下标不能使用 in-out 参数。
...
`Matrix` 提供了一个接受两个入参的构造方法，入参分别是 `rows` 和 `columns`，创建了一个足够容纳 `rows * columns` 个 `Double` 类型的值的数组。通过传入数组长度和初始值 `0.0` 到数组的构造器，将矩阵中每个位置的值初始化为 `0.0`。关于数组的这种构造方法请参考[使用默认值创建数组](/collections/swift/collection-types#使用默认值创建数组)。
...
[← 方法](/collections/swift/methods) · [继承 →](/collections/swift/inheritance)
