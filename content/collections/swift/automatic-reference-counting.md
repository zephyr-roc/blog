...
title: 自动引用计数
date: 2026-09-08
excerpt: 管理对象及其关系的生命周期。
chapter: 语言指南
chapterOrder: 26
---

> 本文迁移自 SwiftGG 中文版《The Swift Programming Language 6.2.3》的[“自动引用计数”](https://doc.swiftgg.team/documentation/the-swift-programming-language/automaticreferencecounting/)，原文由 Apple Inc.、Swift 项目作者及 SwiftGG 译者贡献，依据 Apache License 2.0（含 Runtime Library Exception）发布。本站仅做 Markdown、链接与版式适配。
...
你可以通过将类之间的某些关系定义为弱引用或无主引用，而不是强引用，来解决强引用循环。这个过程在[解决类实例之间的强引用循环](/collections/swift/automatic-reference-counting#解决类实例之间的强引用循环)中描述。然而，在学习如何解决强引用循环之前，了解这种循环是如何产生的是很有用的。
...
![](/swift-guide-assets/referenceCycle01@2x.png)
...
![](/swift-guide-assets/referenceCycle02@2x.png)
...
![](/swift-guide-assets/referenceCycle03@2x.png)
...
![](/swift-guide-assets/weakReference01@2x.png)
...
![](/swift-guide-assets/weakReference02@2x.png)
...
![](/swift-guide-assets/weakReference03@2x.png)
...
![](/swift-guide-assets/unownedReference01@2x.png)
...
![](/swift-guide-assets/unownedReference02@2x.png)
...
![](/swift-guide-assets/unownedOptionalReference@2x.png)
...
`City` 的初始化器是在 `Country` 的初始化器内部调用的。然而，`Country` 的初始化器在新的 `Country` 实例完全初始化之前，不能将 `self` 传递给 `City` 初始化器，如[两段式构造过程](/collections/swift/initialization#两段式构造过程)中所述。

为了应对这个要求，你将 `Country` 的 `capitalCity` 属性声明为隐式解包可选属性，通过在其类型注解末尾加上感叹号(`City!`)来表示。这意味着 `capitalCity` 属性有一个默认值 `nil`，像任何其他可选类型一样，但可以在不需要解包其值的情况下访问，如[隐式解包可选](/collections/swift/the-basics#隐式解包可选)中所述。
...
![](/swift-guide-assets/closureReferenceCycle01@2x.png)

实例的 `asHTML` 属性持有对其闭包的强引用。然而，因为闭包在其主体内引用了 `self`(作为引用 `self.name` 和 `self.text` 的方式)，闭包 *捕获* 了 self，这意味着它持有对 `HTMLElement` 实例的强引用。两者之间创建了一个强引用循环。(关于在闭包中捕获值的更多信息，请参见[值捕获](/collections/swift/closures#值捕获)。)
...
无主引用是解决[闭包的强引用循环](/collections/swift/automatic-reference-counting#闭包的强引用循环) 中 `HTMLElement` 示例强引用循环的适当捕获方法。以下是如何编写 `HTMLElement` 类以避免循环:
...
![](/swift-guide-assets/closureReferenceCycle02@2x.png)
...
关于捕获列表的更多信息，请参见[捕获列表](/collections/swift/expressions#捕获列表)。

---

[← 不透明类型和封装协议类型](/collections/swift/opaque-types) · [内存安全 →](/collections/swift/memory-safety)
