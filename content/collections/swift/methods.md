...
title: 方法
date: 2026-09-08
excerpt: 定义并调用属于某个实例或者类型的函数。
chapter: 语言指南
chapterOrder: 11
---

> 本文迁移自 SwiftGG 中文版《The Swift Programming Language 6.2.3》的[“方法”](https://doc.swiftgg.team/documentation/the-swift-programming-language/methods/)，原文由 Apple Inc.、Swift 项目作者及 SwiftGG 译者贡献，依据 Apache License 2.0（含 Runtime Library Exception）发布。本站仅做 Markdown、链接与版式适配。
...
实例方法有着和函数完全一样的语法，具体描述可以参见 [函数](/collections/swift/functions)。
...
函数参数可以同时拥有一个参数名称（在函数体内使用）和一个参数标签（在调用函数时使用），参见[函数参数标签和参数名称](/collections/swift/functions#函数参数标签和参数名称)。
...
参见：[常量结构体实例的存储属性](/collections/swift/properties#常量结构体实例的存储属性)
...
有关此属性的更多信息，请参见 [特性](/collections/swift/attributes)。
...
[← 属性](/collections/swift/properties) · [下标 →](/collections/swift/subscripts)
