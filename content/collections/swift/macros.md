...
title: 宏
date: 2026-09-08
excerpt: 在编译时使用宏来生成代码。
chapter: 语言指南
chapterOrder: 19
---

> 本文迁移自 SwiftGG 中文版《The Swift Programming Language 6.2.3》的[“宏”](https://doc.swiftgg.team/documentation/the-swift-programming-language/macros/)，原文由 Apple Inc.、Swift 项目作者及 SwiftGG 译者贡献，依据 Apache License 2.0（含 Runtime Library Exception）发布。本站仅做 Markdown、链接与版式适配。

在编译时使用宏来生成代码。

宏会在编译你的源代码时对其进行转换，从而让你避免手动编写重复的代码。在编译过程中，Swift 会先展开代码中的所有宏，然后再像往常一样构建代码。

![一个宏展开示意图。左侧是 Swift 代码的风格化表示。右侧是由宏添加了几行的相同的代码。](/swift-guide-assets/macro-expansion@2x.png)
...
要了解更多信息，包括宏角色的完整列表，请参阅 [特性](/collections/swift/attributes) 中的 [attached](/collections/swift/attributes#attached) 和 [freestanding](/collections/swift/attributes#freestanding)。

## 宏的展开

在构建使用了宏的 Swift 代码时，编译器会调用宏的实现来展开它们。

![显示宏展开的四个步骤的图表。输入是 Swift 源代码。源代码变成了一棵树，代表代码的结构。宏的实现向这棵树添加了新的分支。结果是带有添加过代码的 Swift 源代码。](/swift-guide-assets/macro-expansion-full@2x.png)
...
`#fourCharacterCode` 宏接受一个长度为四个字符的字符串作为输入，并返回一个无符号的 32 位整数，该整数对应于组成字符串的字符的 ASCII 码值的组合。一些文件格式使用这样的整数来标识数据，因为它们紧凑且在调试器中仍然可读。下面的 [实现一个宏](/collections/swift/macros#实现一个宏) 的部分展示了如何实现这个宏。
...
![一个树状图，以常量作为根节点。该常量有一个名为 magicNumber 的名称和一个值。该常量的值是一个宏调用。这个宏调用有一个名为 fourCharacterCode 的名称和它的参数。参数是一个值为 ABCD 的字符串字面量。](/swift-guide-assets/macro-ast-original@2x.png)
...
![一个树状图，以一个宏调用（Macro call）作为根节点。这个宏调用有一个名为 fourCharacterCode 的名称和参数。这个参数是一个值为 ABCD 的字符串字面量。](/swift-guide-assets/macro-ast-input@2x.png)
...
![一个具有 UInt32 类型的整型字面量 1145258561 的树形图。](/swift-guide-assets/macro-ast-output@2x.png)
...
![一个树状图，以常量作为根节点。该常量有一个名为 magicNumber 的名称和一个值。该常量的值是 UInt32 类型的整型字面量 1145258561。](/swift-guide-assets/macro-ast-result@2x.png)
...
`#fourCharacterCode` 宏是一个产出一个表达式的独立宏，因此，实现它的 `FourCharacterCode` 类型需遵循 `ExpressionMacro` 协议。`ExpressionMacro` 协议有一个要求，即有一个 `expansion(of:in:)` 方法来展开 AST。有关宏的角色列表及其相应的 SwiftSyntax 协议，请参阅 [特性](/collections/swift/attributes) 中的 [attached](/collections/swift/attributes#attached) 和 [freestanding](/collections/swift/attributes#freestanding)。
...
[← 并发](/collections/swift/concurrency) · [类型转换 →](/collections/swift/type-casting)
