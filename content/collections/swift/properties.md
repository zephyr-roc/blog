...
title: 属性
date: 2026-09-08
excerpt: 访问属于实例或类型的存储值和计算值。
chapter: 语言指南
chapterOrder: 10
---

> 本文迁移自 SwiftGG 中文版《The Swift Programming Language 6.2.3》的[“属性”](https://doc.swiftgg.team/documentation/the-swift-programming-language/properties/)，原文由 Apple Inc.、Swift 项目作者及 SwiftGG 译者贡献，依据 Apache License 2.0（含 Runtime Library Exception）发布。本站仅做 Markdown、链接与版式适配。
...
可以在定义存储属性的时候指定默认值，请参考 [默认属性值](/collections/swift/initialization#默认属性值) 一节。还可以在构造过程中设置和修改存储属性的初始值，甚至修改常量存储属性的值，请参考 [构造过程中常量属性的赋值](/collections/swift/initialization#构造过程中常量属性的赋值) 一节。
...
![](/swift-guide-assets/computedProperties@2x.png)
...
省略 getter 中的 `return` 遵循与函数省略 `return` 相同的规则，详见 [隐式返回的函数](/collections/swift/functions#隐式返回的函数)。
...
对于继承的属性，可以通过在子类中重写该属性来添加属性观察器。对于自定义的计算属性，应使用属性的 setter 来观察和响应值的变化，而不是试图创建一个观察器。重写属性的相关内容详见 [重写](/collections/swift/inheritance#重写)。
...
>
> 有关初始化器代理的更多信息，请参见 [值类型的构造器代理](/collections/swift/initialization#值类型的构造器代理) 和 [类类型的构造器代理](/collections/swift/initialization#类类型的构造器代理)。
...
> 注意: 
> 如果将一个具有观察器的属性作为 in-out 参数传递给函数，那么 `willSet` 和 `didSet` 观察器总是会被调用。这是因为 in-out 参数的复制-写回内存模型：在函数结束时，值总是会被写回属性。关于 in-out 参数行为的详细讨论，请参见 [In Out 参数](/collections/swift/declarations#In-Out-参数)。
...
> 注意: 
> 上例中 `number` 的声明被标记为 `private`，这确保了 `number` 只能在 `TwelveOrLess` 的实现中使用。其他地方的代码只能通过 `wrappedValue` 的 getter 和 setter 来访问这个值，而不能直接使用 `number`。关于 `private` 的更多信息，请参见 [访问控制](/collections/swift/access-control)。
...
`SmallNumber` 的定义包括三个构造器——`init()`、`init(wrappedValue:)` 和 `init(wrappedValue:maximum:)`——下面的示例使用这些构造器来设置被包装值和最大值。关于初始化和构造器语法的更多信息，请参见 [构造过程](/collections/swift/initialization)。
...
> 注意: 
> 全局常量和变量总是以类似于 [延时加载存储属性](/collections/swift/properties#延时加载存储属性) 的方式被延迟计算。与延迟存储属性不同，全局常量和变量不需要用 `lazy` 修饰符标记。
>
> 局部常量和变量从不延迟计算。
...
![](/swift-guide-assets/staticPropertiesVUMeter@2x.png)
...
[← 结构体和类](/collections/swift/classes-and-structures) · [方法 →](/collections/swift/methods)
