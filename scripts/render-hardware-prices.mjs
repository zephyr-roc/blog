...
const latestObservationDate = data.series
  .flatMap((series) => series.observations.map((item) => item.date))
  .sort()
  .at(-1);
const reportDate = process.argv[2] ?? latestObservationDate;
if (!/^\d{4}-\d{2}-\d{2}$/.test(reportDate) || reportDate < latestObservationDate) {
  throw new Error(`报告日期必须不早于最后观测日 ${latestObservationDate}`);
}
const outputPath = path.join(root, `public/deep-radar/${reportDate}-cn-hardware-prices.svg`);
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="title desc">
  ...
  <desc id="desc">${startDate} 至 ${reportDate} CPU、GPU、主板、DRAM、HDD 与 SSD 的可复核报价。只连接真实日期点，单点品类不生成趋势线。</desc>
  <defs>
  ...
  <text x="30" y="75" class="subtitle">${startDate}—${reportDate} · 公开可复核价格事件 · 单位：人民币 · 不插值</text>
  ${panels}
