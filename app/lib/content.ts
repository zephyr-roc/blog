...
function prepareMermaidSvg(source: string) {
  const prefix = `mermaid-${contentHash(source)}`;
  let svg = renderMermaidSVG(source, {
    bg: "var(--reading-background, var(--night))",
    fg: "var(--ink)",
    line: "color-mix(in srgb, var(--ink) 48%, transparent)",
    accent: "color-mix(in srgb, var(--ink) 58%, transparent)",
    muted: "color-mix(in srgb, var(--ink) 64%, transparent)",
    surface: "var(--post-glass-surface-strong)",
    border: "color-mix(in srgb, var(--ink) 34%, transparent)",
    font: "JetBrains Mono",
    transparent: true,
    padding: 24,
  });

  // The site already ships JetBrains Mono locally. Do not make each SVG fetch
  // the Google Fonts stylesheet generated by the renderer.
  svg = svg.replace(/\s*@import url\([^;]+;\s*/, "\n  ");

  // beautiful-mermaid emits stable IDs such as `arrowhead` and node IDs.
  // Scope them per diagram so multiple server-rendered SVGs can coexist.
  ...
  return svg.replace(
    "<svg ",
    '<svg aria-hidden="true" focusable="false" data-mermaid-svg="true" ',
  );
}
...
marked.use({
  renderer: {
    code({ text, lang }) {
      ...
      if (language === "mermaid") {
        try {
          const svg = prepareMermaidSvg(text);
          return `<figure class="mermaid-diagram" data-mermaid-diagram data-mermaid-rendered="true"><div class="mermaid-diagram__canvas" role="img" aria-label="文章流程图">${svg}</div><figcaption class="sr-only">文章流程图</figcaption></figure>\n`;
        } catch {
          return `<figure class="mermaid-diagram" data-mermaid-diagram data-mermaid-error="true"><pre class="mermaid-diagram__source"><code class="language-mermaid">${escapeHtml(text)}</code></pre><figcaption class="sr-only">Mermaid 图表源码</figcaption></figure>\n`;
        }
      }
